...
/*   Updated: 2024/11/26 16:01:38 by anilchen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_shell_context *shell_ctx)
{
	...
	if (cmd->redirections)
	{
		...
		{
			ft_putstr_fd("Error handling redirections\n", 2);
			exit(EXIT_FAILURE);
		}
	}
	if (is_builtin(cmd))
		...
		t_pipes_process_content *cmd_ctx)
{
	...
	if (create_fork(&pid, cmd_ctx->shell_ctx->process) == -1)
		return (-1);
	if (pid == 0)
...
// 	{
// 		handle_streams(fds, PIPE_LAST);
// 		execute_builtin(cmd, shell_ctx->env_copy,
//				shell_ctx->process);
// 		exit(shell_ctx->process->last_exit_status);
// 	}
// 	if (is_executable(cmd->args[0]))
